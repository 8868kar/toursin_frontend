...
export default function User() {
    ...
    const handleSearch = () => {
        ...
        const matchingTour = POSTERS.find(tour =>
            tour.title.toLowerCase().includes(searchDestination.toLowerCase()) ||
            tour.description.toLowerCase().includes(searchDestination.toLowerCase())
        );

        if (matchingTour) {
            navigate(`/tour/${matchingTour.id}`);
        } else {
            ...
            <section id="tours" className="relative pt-12 pb-32 px-4 overflow-hidden flex flex-col items-center justify-start min-h-[85vh]">
                ...
                <div className="max-w-7xl mx-auto w-full">
                    ...
                    <div className="bg-white/90 dark:bg-slate-900/90 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-4 md:p-8 max-w-5xl mx-auto relative z-20 border border-slate-100/50 dark:border-slate-700/50 backdrop-blur-sm">
                        ...
                        {/* Search Input Grid */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 relative shadow-sm">
                            ...
                            {/* MONTH */}
                            <div className="relative border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800 pb-4 md:pb-0 md:px-4 group cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 -m-4 md:m-0 p-4 transition-colors">
                                <label className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover:text-emerald-500 transition-colors">
                                    Travel Month
                                </label>
                                <div className="text-3xl font-black text-slate-900 dark:text-white flex items-baseline gap-1 mt-1">
                                    April <span className="text-sm font-semibold text-slate-500">'26</span>
                                </div>
                                <div className="text-sm font-medium text-slate-500 mt-1">Spring Season</div>
                            </div>

                            {/* TRAVELERS */}
                            <div className="relative pt-2 md:pt-0 md:pl-4 group cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 -m-4 md:-ml-4 p-4 md:pl-8 rounded-r-2xl transition-colors">
                                <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover:text-emerald-500 transition-colors">Guests</label>
                                <div className="text-3xl font-black text-slate-900 dark:text-white flex items-baseline gap-1 mt-1">
                                    2 <span className="text-xl font-bold">People</span>
                                </div>
                                <div className="text-sm font-medium text-slate-500 truncate mt-1">1 Room</div>
                            </div>
                        </div>
            ...
            <section id="packages" className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full pt-16 pb-12 relative z-10">
                <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white dark:border-slate-800 p-6 md:p-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* Offer 1 */}
                        <div className="bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 flex gap-4 hover:shadow-md transition-shadow cursor-pointer group">
                            <img src="https://images.unsplash.com/photo-1595815771614-ade9d6527653?w=150&h=150&fit=crop" className="w-24 h-24 rounded-xl object-cover shadow-sm group-hover:scale-105 transition-transform" />
                            <div className="flex-1 flex flex-col justify-center">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Compass size={12} /> SUMMER SPECIAL</p>
                                <h4 className="font-bold text-slate-900 dark:text-white mb-2 leading-tight text-lg">Up to ₹5000 OFF on Kashmir Tours!</h4>
                                <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                                    <span className="font-bold border border-dashed border-emerald-300 dark:border-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded">NORTH26</span>
                                </p>
                            </div>
                        </div>
                        {/* Offer 2 */}
                        <div className="bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 flex gap-4 hover:shadow-md transition-shadow cursor-pointer group">
                            <img src="https://images.unsplash.com/photo-1561359313-0639aad48e23?w=150&h=150&fit=crop" className="w-24 h-24 rounded-xl object-cover shadow-sm group-hover:scale-105 transition-transform" />
                            <div className="flex-1 flex flex-col justify-center">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Compass size={12} /> HOLIDAY PACKAGES</p>
                                <h4 className="font-bold text-slate-900 dark:text-white mb-2 leading-tight text-lg">Flat 15% OFF on Varanasi Tours.</h4>
                                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">With HDFC Credit Cards</p>
                            </div>
                        </div>
                        {/* Offer 3 */}
                        <div className="bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 gap-4 hover:shadow-md transition-shadow cursor-pointer group hidden lg:flex">
                            <img src="https://images.unsplash.com/photo-1595815771614-ade9d6527653?w=150&h=150&fit=crop" className="w-24 h-24 rounded-xl object-cover shadow-sm group-hover:scale-105 transition-transform" />
                            <div className="flex-1 flex flex-col justify-center">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Search size={12} /> APP EXCLUSIVE</p>
                                <h4 className="font-bold text-slate-900 dark:text-white mb-2 leading-tight text-lg">Get 10% Extra Discount on Mobile</h4>
                                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No code required</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section id="destinations" className="py-24 w-full bg-slate-950 relative mt-12 rounded-t-[3rem] shadow-[0_-20px_40px_rgba(0,0,0,0.1)]">
                ...
                <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full relative z-10">
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 min-h-[600px]">
                        {POSTERS.map((poster) => {
                            const isActive = activeId === poster.id;
                            return (
                                <div
                                    key={poster.id}
                  ...
                  ${isActive ? 'lg:flex-[3] flex-1 shadow-[0_20px_50px_rgba(0,0,0,0.5)] scale-[1.02] h-[28rem] lg:h-[600px] z-20 ring-1 ring-emerald-500/50' : 'lg:flex-[1] h-40 lg:h-[600px] shadow-lg opacity-70 hover:opacity-100 z-10'}`}
                                >
                                    {/* Background Image */}
                                    <img
                                        src={poster.image}
                                    ...
                                    <div className={`absolute inset-0 bg-gradient-to-t ${poster.accentColor} transition-opacity duration-500 ${isActive ? 'opacity-90' : 'opacity-60 mix-blend-multiply'}`} />
                                        ...
                                        <div className={`transform transition-all duration-700 ease-in-out w-full ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0 lg:opacity-100'}`}>
                                            <div className="flex flex-wrap items-center gap-2 mb-4">
                                                <span className="bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest text-white flex items-center gap-1 border border-white/20 shadow-sm">
                                                    <MapPin size={14} /> {poster.title.split(' ')[1]}
                                                </span>
                                                <span className="bg-yellow-400/20 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold text-yellow-100 flex items-center gap-1 border border-yellow-400/30 shadow-sm">
                                                    <Star size={14} className="text-yellow-400 fill-yellow-400" /> {poster.rating}
                                                </span>
                                            </div>
                                            ...
                                            <div className={`mt-2 space-y-4 overflow-hidden transition-all duration-500 delay-100 ${isActive ? 'max-h-64 lg:max-h-48 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                ...
                                                <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/20">
                                                    <div>
                                                        ...
                                                        <p className="text-4xl font-black drop-shadow-md">{poster.price}</p>
                                                    </div>
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); navigate(`/tour/${poster.id}`); }}
                                                        className="bg-white text-slate-900 rounded-full h-16 w-16 flex items-center justify-center hover:scale-110 transition-transform shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:shadow-[0_0_40px_rgba(255,255,255,0.6)] group/btn relative overflow-hidden"
                                                    >
