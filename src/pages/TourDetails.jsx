...
export default function TourDetails() {
    ...
    useEffect(() => {
        // Find the specific tour in our data based on the URL parameter ID
        window.scrollTo(0, 0);
        const selectedTour = POSTERS.find(p => p.id === parseInt(id));
        if (selectedTour) {
            setTour(selectedTour);
        }
    }, [id]);

    if (!tour) return <div className="min-h-screen flex items-center justify-center pt-20">Loading Tour Details...</div>;

    // Convert string price to number for calculation
    const basePrice = parseInt(tour.price.replace(/[^0-9]/g, ''));
                ...
                <div className="absolute inset-0 flex flex-col justify-end pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full text-white">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-4">
                        <span className="bg-emerald-500/90 backdrop-blur-md px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wider flex items-center gap-2">
                            <MapPin size={16} /> {tour.title.split(' ')[1] || tour.title}
                        </span>
                    ...
                    <p className="text-xl md:text-2xl text-white/90 max-w-2xl">{tour.description}</p>
                </div>
            </div>
                                ...
                                <ul className="space-y-3">
                                    {tour.inclusions?.map((inc, i) => (
                                        <li key={i} className="flex items-start gap-3 text-slate-600 font-medium">
                                            <div className="mt-1 bg-emerald-100 p-1 rounded-full"><Check size={12} className="text-emerald-600" /></div>
                                            {inc}
                                        </li>
                                    ))}
                                </ul>
                                ...
                                <ul className="space-y-3">
                                    {tour.exclusions?.map((exc, i) => (
                                        <li key={i} className="flex items-start gap-3 text-slate-500">
                                            <div className="mt-1 bg-red-100 p-1 rounded-full"><X size={12} className="text-red-500" /></div>
                                            {exc}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            {tour.itinerary?.map((day, index) => (
                                <div key={index} className="flex gap-6">
                                    ...
                                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex-1 mb-2 hover:shadow-md transition-shadow">
                                        ...
                                        <p className="text-slate-600 leading-relaxed font-medium">{day.details}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="lg:w-1/3 relative">
                    <div className="sticky top-28 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                        ...
                        <div className="space-y-6 mb-8">
                            {/* Date Picker Input (Visual Only for now) */}
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">Select Date</label>
                                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl p-4 cursor-pointer hover:border-emerald-500 transition-colors">
                                    <Calendar className="text-emerald-600" size={20} />
                                    <span className="font-semibold text-slate-700">Select Travel Date</span>
                                </div>
                            </div>
